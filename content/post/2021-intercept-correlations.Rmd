...
datasets_c <- generate_datasets(generator_c, n_datasets = 100)
...
datasets_c_2 <- generate_datasets(generator_c, n_datasets = 300)

SBC_res_suff_c_2 <- 
  bind_results(SBC_res_suff_c,
  compute_results(datasets_c_2, suff_c_backend, 
                                  gen_quants = generated_quantities(
                                    mu_r = mean(group_r),
                                    group_r_c = group_r - mean(group_r),
                                    intercept_c = intercept + mean(group_r),
                                  ...
                                  keep_fits = FALSE,
                                  cache_mode = "results",
                                  cache_location = file.path(SBC_cache_dir, "suff_c_datasets_c_2.rds"))
  )
```


```{r}
plot_ecdf_diff(SBC_res_suff_c_2)
plot_ecdf_diff(SBC_res_suff_c_2[SBC_res_suff_c_2$backend_diagnostics$n_divergent == 0])
plot_rank_hist(SBC_res_suff_c_2)
plot_rank_hist(SBC_res_suff_c_2[SBC_res_suff_c_2$backend_diagnostics$n_divergent == 0])

```

```{r}
# SBC_res_nc <- compute_results(datasets_c, SBC_backend_cmdstan_sample(model_nc), cache_mode = "results",
#                                   cache_location = file.path(SBC_cache_dir, "nc_datasets_c.rds"))
```
```{r}
# plot_ecdf_diff(SBC_res_nc)
# plot_rank_hist(SBC_res_nc)
```
...
parameters {
  ...
  vector[N_groups - 1] group_z_raw; 
  real mean_group_z;
  real<lower=0> group_sd;
}

transformed parameters {
  vector[N_groups] group_z_sweep;
  vector[N_groups] group_z;
  vector[N_groups] group_r_sweep;
  group_z_sweep[1:N_groups - 1] = group_z_raw;
  group_z_sweep[N_groups] = -sum(group_z_raw);
  group_r_sweep = group_z_sweep * group_sd;
  
  //This will need Jacobian
  intercept = intercept_sweep + mean_group_z * group_sd;
}
...
generated quantities {
  vector[N_groups] group_z = group_z_sweep + mean_group_z;
  vector[N_groups] group_r = group_z = group_z * group_sd; 
