...
transformed parameters {
  vector[N_groups] group_r_sweep_z = sum_to_zero_QR(group_r_raw, groups_Q_r);
  vector[N_groups] group_r_sweep = group_r_sweep_z * group_sd;
}

model {
  matrix[N_groups + 1, N_groups + 1] prior_sigma =
    quad_form(diag_matrix(append_row([sigma_alpha^2]', rep_vector(1, N_groups))), transpose(inv_A));
  
  vector[N_groups + 1] intercept_and_groups = append_row([intercept_sweep]', group_r_sweep_z[1:(N_groups - 1)]);
...
generated quantities {
  ...
  m_R = (intercept_sweep - mu_alpha) / (N_groups * sigma_alpha^2 + 1);
  sigma_R = sqrt(inv(inv(sigma_alpha^2) + N_groups));

  mean_group_r = normal_rng(m_R, sigma_R);
  group_r = (group_r_sweep_z + mean_group_r) * group_sd;
  intercept = intercept_sweep - mean_group_r * group_sd;
}
