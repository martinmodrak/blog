...
Spoiler: it turns out the saddlepoint approximation is only slightly better than a simpler alternative.
...
```{r plot_approx_fun, cache=TRUE }
expose_stan_functions(here("static","post","2019-saddlepoint-approximation","sum_nb_functions.stan"))
...
#Choose how the means are generated and how many NB variables are there
  
  n_means = length(means)

    #data = generate_nb_sum(10000, n_means, phi, means = rlnorm(n_means, 8, 5))
  #n_means = 10
...
```
It is visible that the saddlepoint mass track the empirical mass very tightly both in the bulk and in the tail (visible better on the log mass) - note that the tail of the empirical log mass is jittery due to low number of samples in the tail.
...
Here we test a sum of two NBs - the means of the unobserved NB is chosen randomly from LogNormal(`r mu_prior_mean_small`,`r mu_prior_sd_small`) and the observed mean is chosen from LogNormal(`r observed_mean_mus_small`,`r observed_sd_mus_small`). The observed mean is deliberately smaller on average as when the observed mean is much larger than the mean to be inferred, there is little to be learned about the unobserved mean except that is not big and prior dominates making the case uninteresting. We observe `r small_N` sums.
...
As in the above plot, we see that $\mu_{G+1}$ is usually inferred quite precisely, while the results for $\phi_{G+1}$ are diffuse, often dominated by the priors and have a slight tendency to be below the perfect prediction line.
...
Further, we can check the case where there are 20 known variables with low means and one NB is unknown with a large mean - once again we want the unobserved mean to have notable influence on the total outcome, hence we choose it to be larger. In particular, the observed means are drawn from LogNormal(`r observed_mean_mus_large`,`r observed_mean_sd_large`) and the mean to be inferred is drawn from LogNormal(`r mu_prior_mean_large`,`r mu_prior_sd_large`)
...
We see that in the regimes we tested, the saddlepoint approximation for sum of negative binomials provides  somewhat better inferences at the cost of much increased computation times. So it is probably not very practical unless you need that extra bit of precision. But it is a neat mathematical trick and of interest on its own. It is also possible that for some low-mean regimes the difference is bigger.
