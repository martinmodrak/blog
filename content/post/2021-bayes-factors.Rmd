...
# Running the model
model <- jags.model( textConnection(jags_model_string), data_list, n.chains = 4, n.adapt= 1000)
...
res_jags <- data.frame(method = "full_jags", bf = bf_jags, prob_null = prob_null_jags)
...
res_stan <- data.frame(method = "full_stan", bf = bf_stan, prob_null = prob_null_stan)
...
If we treat the model selection as fitting the full model with the candidates as submodels, being in an $\mathcal{M}$-open scenario simply means that the full model is misspecified. Unfortunately, as discussed above, Bayes factors can be very sensitive to both prior and likelihood specification, where other approaches, e.g. approximate leave-one-out cross validation with `loo` is usually less sensitive to priors and less often overconfident in face of this misspecifaction (see e.g.  [Using Stacking to Average Bayesian Predictive Distributions](https://projecteuclid.org/journals/bayesian-analysis/volume-13/issue-3/Using-Stacking-to-Average-Bayesian-Predictive-Distributions-with-Discussion/10.1214/17-BA1091.full) by Yao et al. for more discussion).

But we have many good ways to detect model misspecification, including prior and posterior predictive checks and other steps outlined in the [Bayesian workflow preprint](https://arxiv.org/abs/2011.01808). So here is a free research idea (if it is not already done - I don't know the literature very well): implement prior and posterior predictive checks (or other diagnostics), not only for the component models, but for the full model implied by the selection scheme. 


## Broader comparison

Above, we computed the Bayes factors only 
...
bf_full_stan <- function(y) {
  ...
  data.frame(method = "full_Stan", bf = bf, prob_null = p_null)
}

bf_full_jags <- function(y) {
  data_list <- list(
    y = y)
  params_to_monitor <- c("z_0", "alpha")
  ...
  bf_jags <- prob_null_jags / (1 - prob_null_jags)
  bf_jags
  data.frame(method = "full_jags", bf = bf_jags, prob_null = prob_null_jags)
}
...
if(!file.exists(comparisons_file)) {
  ...
  for(i in 1:50) {
    ...
    y <- rnorm(10, mu, sd = 1)
    res_list[[i]] <- rbind(bf_prior(y), bf_brms(y), bf_full_jags(y), bf_full_stan(y)) 
    res_list[[i]]$id = i
...
comparisons_to_plot <- res %>% group_by(id) %>%
  mutate(bf_prior = bf[method == "prior"], prob_null_prior = bf[method == "prior"]) %>%
  ungroup() %>%
  filter(method != "prior")

zero_bf <- comparisons_to_plot %>% filter(bf == 0)
if(!identical(unique(zero_bf$method), "full_jags")){
  stop("Bad assumption")
}
nrow(zero_bf)
...
```{r}
base_comparisons_plot(comparisons_to_plot %>% filter(bf_prior > 1e-3), bf_prior, bf, trans = "log",
...
Since the model has analytical solution, our simulations are going to be much cheaper than in actual practice.
