...
To make the model work with SBC we make a couple tweaks. 
First, we add the `0 + Intercept` term to avoid the default centering of predictors done by `brms`,
second we add explicit priors for all parameters (as the simulations need to match
those priors). We'll use $N(0,1)$ for most parameters except the intercept for
random effect deviations ($\alpha^\prime$) where we'll use $N(1,1)$ to avoid very low 
standard deviations which pose convergence problems. Similarly, very low observation sigma
causes convergence problems (not completely sure why), so we'll use a $\Gamma(`r gamma_shape`, `r gamma_rate`)$ prior (roughly saying that a priori
the standard deviation is unlikely to be less than `r round(qgamma(0.025, gamma_shape, gamma_rate), 1)` or
more than `r round(qgamma(0.975, gamma_shape, gamma_rate), 1)` )

```{r, message=FALSE, warning=FALSE}
# f_sbc <- 
#   mvbrmsformula(
#     brmsformula(y | subset(!is_trt)  ~ 0 + Intercept + x, family = empty_for_obs),
#     brmsformula(trt_y | subset(is_trt)  ~ 0 + Intercept + trt_x, family = empty_for_trt),
#     rescor = FALSE)

#get_prior(f_sbc, combined_data)

priors <- c(
  prior(normal(0,1), class = "b", resp = "trty"),
  prior(normal(1,1), class = "b", coef = "Intercept", resp = "trty"),
...
generator_func <- function() {
  trt_intercept <- rnorm(1, mean = 1)
  trt_x_b <- rnorm(1)
  # Centering predictors to match brms
  trt_x_b <- trt_x_b - mean(trt_x_b)
...
# With 500 datasets, this takes ~30 minutes on my computer
backend <-
  SBC_backend_brms(f,
                   stanvars = predict_ranef_stanvars,
                   prior = priors,
...
dq <- derived_quantities(
  lprior_fixed = dnorm(b_y_Intercept, log = TRUE) +
    ...
    dnorm(b_trty_Intercept, mean = 1, log = TRUE) +
...
plot_ecdf_diff(sbc_res, variables = vars)
```
...
```{r sbc-sim-est}
plot_sim_estimated(sbc_res, variables = vars, alpha = 0.1)
```

```{r}
fit_to_test <- SBC_fit(backend, ds$generated[[303]], 4)
```

```{r}
pairs(fit_to_test$fit, pars = vars[c(1:5, 8,9)])
```

```{r}
ds$variables[210,]
ds$generated[[210]] %>% filter(is_trt) %>% pull(trt_x) 
ds$generated[[210]] %>% filter(is_trt) %>% pull(trt_x) %>% mean()
```



```{r}
sbc_res$stats %>% filter(rhat < 1.5, rhat > 1.1
                         )
```


```{r}
#sbc_res$stats %>% filter(sim_id == 17, !grepl("my_", variable))
sbc_res$stats %>% filter(!grepl("my_", variable)) %>% inner_join(sbc_res$default_diagnostics, by = "sim_id") %>%
  #filter(max_rhat < 1.5) %>%
  ggplot(aes(x = simulated_value, y = max_rhat)) + geom_point() + facet_wrap(~variable, scales = "free_x")
```
...
Or alternatively, the `renv` lock file to run this:
